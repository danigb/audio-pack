...
var noImages = new marked.Renderer()
noImages.image = function (href, title, text) { return '' }
...
function htmlPackIndex (files) {
  var packs = Object.keys(files).map(function (name) {
    ...
    return {
      ...
      url: REPO + config.name + '/audio-pack.json',
      urlMeta: REPO + config.name + '/meta-audio-pack.json',
      readme: marked(config.readme, { renderer: noImages })
    }
  })
...
function htmlPack (name, files) {
  var output = {}
  var meta = files[name + '/meta-audio-pack.json'].contents.toString()
  var data = JSON.parse(meta)
  data.readme = marked(data.readme)
  data.audioUrl = REPO + name + '/audio-pack.json'
  data.metaUrl = REPO + name + '/meta-audio-pack.json'
  data.self = meta
  output[name + '/index.html'] = { contents: new Buffer(templates.pack(data)) }
  return output
}

function bufToString (buffer) { return buffer ? buffer.contents.toString() : null }
function bufJsonToObj (buffer) { return buffer ? JSON.parse(bufToString(buffer)) : null }

function publish (name, files) {
  var output = {}
  var audioFiles = filter(isAudio(), files)
  var config = bufJsonToObj(files[name + '/instrument.json'])
  var readme = bufToString(files[name + '/README.md'])
  output[name + '/audio-pack.json'] = objToBuffer(buildPack(audioFiles))
  output[name + '/meta-audio-pack.json'] = objToBuffer(buildMeta(config, readme, audioFiles))
  return output
}

var REPO = 'http://danigb.github.io/audio-pack/published/'
function buildMeta (config, readme, audioFiles) {
  ...
  meta.readme = readme ? readme.slice(readme.indexOf('\n')) : ''
  meta.repository = REPO + meta.name
  meta.pack = meta.repository + '/audio-pack.json'
  return meta
}
